...
export class GenerateAllowanceRequestComponent implements OnInit {
  ...
  async loadData() {
    if (this.idSolicitudAuxilio) {
      ...
      if (this.lineaCredito) {  //CAMBIARRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR
        const lineaNombre = await firstValueFrom(
          this.allowanceTypeService.getNameById(this.lineaCredito)
        );
    ...
    if (this.userId) {
      ...
      const [expCity, birthCity, residenceCity, departments] =
      ...
      const conyuge = familyData.find(
        (f: any) => f.idParentesco === 5 || f.idParentesco === 6
      );
      if (conyuge) {
  ...
  async generateExcel() {
    ...
    try {
      ...
      if (worksheet) {
        ...
        worksheet.getCell('K6').value = Number(this.idSolicitudAuxilio);
        ...
        worksheet.getCell('F13').value = `${this.ciudadExpedicionDocumento}, ${
          this.fechaExpedicionDocumento
            ? new Date(
                this.fechaExpedicionDocumento + 'T00:00:00'
              ).toLocaleDateString()
            : ''
        }`;
        ...
        worksheet.getCell(
          'R21'
        ).value = `Teléfono Cónyuge: ${this.telefonoConyuge}`;
        ...
        worksheet.getCell(
          'A32'
        ).value = `Concepto Otros Ingresos: ${this.conceptoOtrosIngresos}`;
  ...
  async onGenerateClick() {
    ...
    try {
      ...
      await this.requestAllowanceService.downloadAllowanceRequestPdf(
        this.idSolicitudAuxilio,
